...
atb_exposure_function <- function(atb) {
  case_when(
    is.na(atb) ~ NA_character_,
...
atb.mdro.table <- map_dfr(1:4, ~{
  ...
  mutate(
    atbdays = as.numeric(end - start),
    atb_class = atb_exposure_function(atb),
...
```{r Generalized Estimating Equations (GEE)}
...
```{r}
#just for fun... trying Bayesian model..

library(brms)

bayes_re_model <- brm(
  CRO ~ age + hyper.factor + renal_insuff.factor + smoke.factor +
    lvad.era + Carbapenems + AntiMRSA + (1 | study_id),
  family = bernoulli(link = "logit"),
  data = gee.atb.mdro.30.data, 
  prior = c(
    set_prior("normal(0, 2.5)", class = "b"),
    set_prior("student_t(3, 0, 2.5)", class = "sd")  # for random effect
  ),
  chains = 4, iter = 2000, warmup = 500, cores = 4,
  seed = 42
)

summary(bayes_re_model)
...
summary_data <- posterior_summary(bayes_re_model)

# Exponentiate fixed effect estimates to get odds ratios
posterior_fixed <- as.data.frame(fixef(bayes_re_model))  # only fixed effects
posterior_fixed$OR <- exp(posterior_fixed$Estimate)
posterior_fixed$OR_Lower <- exp(posterior_fixed$Q2.5)
posterior_fixed$OR_Upper <- exp(posterior_fixed$Q97.5)

posterior_fixed
flextable(posterior_fixed)
